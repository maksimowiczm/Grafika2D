...
GtkWidget *rgb_container_new(ColorsContext *context) {
  ...
  gtk_box_append(GTK_BOX(container), gtk_ui_uint8_picker("red", &rgb->red, 255));
  gtk_box_append(GTK_BOX(container), gtk_ui_uint8_picker("green", &rgb->green, 255));
  gtk_box_append(GTK_BOX(container), gtk_ui_uint8_picker("blue", &rgb->blue, 255));
